  ...
  */
    ...
    while (u32Length > 0U)
    {
        ...
        if ((Fls_u8LocationWritesDataRegs == 0U) && (u32Length >= 128U))
        {
            ...
            if ((uint16)FLS_JOB_OK != ((uint16)FLS_WELL_KNOWN_SECURE_COUNTER ^ (uint16)u16StatusProgram))
            {
                /*had Errors when write*/
                /* @violates @ref FlashProgram_c_REF_1 A function should have a single point of exit at the end */
                return (Fls_CheckStatusType)((uint16)FLS_WELL_KNOWN_SECURE_COUNTER ^ (uint16)u16StatusProgram);
            }
            else
            {
...
static uint16 FlashProgrambytes(uint32 u32StartAddress, Fls_ProgSizeType eWriteMode, uint8 u8DomainIdValue)
{
    ...
    if ((boolean)STD_ON == bEnableTimeOut)
    {
        ...
        else
        {
            /* There were hackers who skipped some instructions */
            /* @violates @ref FlashProgram_c_REF_1 A function should have a single point of exit at the end */
            return (uint16)((uint16)FLS_JOB_FAILED ^ (uint16)FLS_WELL_KNOWN_SECURE_COUNTER);
        }
        ...
        if (0U == u32ValueTimeOut)
        {
            /*Errors Timeout because wait for the Done bit long time*/
            /* @violates @ref FlashProgram_c_REF_1 A function should have a single point of exit at the end */
            return (uint16)((uint16)FLS_TIMEOUT_FAILED ^ (uint16)FLS_WELL_KNOWN_SECURE_COUNTER);
        }
    }
    else
    {
    ...
    if ((boolean)STD_ON == bEnableTimeOut)
    {
        ...
        */
        ...
        else
        {
            /* There were hackers who skipped some instructions */
            /* @violates @ref FlashProgram_c_REF_1 A function should have a single point of exit at the end */
            return ((uint16)FLS_JOB_FAILED ^ (uint16)FLS_WELL_KNOWN_SECURE_COUNTER);
        }
        ...
        if ((0U != REG_BIT_GET32(FLASH_MCRS_ADDR32, FLASH_MCRS_PEG_U32)) && (0U != u32ValueTimeOut))
        {
            ...
            */
            if (0U != REG_BIT_GET32(FLASH_MCRS_ADDR32, FLASH_MCRS_PES_W1C | FLASH_MCRS_PEP_W1C))
            {
                ...
                u16ReturnCode = (uint16)((uint16)FLS_JOB_FAILED ^ (uint16)FLS_WELL_KNOWN_SECURE_COUNTER);
            }
            else
            {
                /* Programed successfully */
                u16ReturnCode = (uint16)((uint16)FLS_JOB_OK ^ (uint16)FLS_WELL_KNOWN_SECURE_COUNTER);
            }
        }
        else
        {
            ...
            if (0U == u32ValueTimeOut)
            {
                u16ReturnCode = (uint16)((uint16)FLS_TIMEOUT_FAILED ^ (uint16)FLS_WELL_KNOWN_SECURE_COUNTER);
            }
